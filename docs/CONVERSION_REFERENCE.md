...
Converter uses:
1. **Locating the main document part robustly**: the part whose content type is one of the four WordprocessingML main-document types — `...wordprocessingml.document.main+xml` for `.docx`, `...ms-word.document.macroEnabled.main+xml` for `.docm`, `...wordprocessingml.template.main+xml` for `.dotx`, and `...ms-word.template.macroEnabledTemplate.main+xml` for `.dotm`. This is the fallback/cross-check for the relationship-based lookup in 1.3.
...
| 6 | Strikethrough | `w:strike`/`w:dstrike` | `~~text~~` | GFM extension. `~~` cannot wrap another delimiter next to a word character — see §4.2 pitfall 11 — so a strikethrough around emphasis or code is `<del>` |
...
| 11 | Inline code | (a) `w:rStyle` resolving to a style named `Code`/`HTML Code`/`Verbatim Char` (Pandoc)/`Source Text` (LibreOffice)/`Macro Text`; (b) effective `rFonts/@ascii` in monospace set: Consolas, Courier, Courier New, Cascadia Code/Mono, Lucida Console, Menlo, Monaco, DejaVu Sans Mono, Liberation Mono, Fira Code/Mono, JetBrains Mono, Source Code Pro, Roboto Mono, Ubuntu Mono, IBM Plex Mono | `` `code` `` | Inside code spans nothing is escaped: if content contains backticks, lengthen the fence (`` ``a`b`` ``) and pad with spaces when content starts/ends with a backtick. Code detection wins over bold/italic (drop them) or nests HTML — pick: drop, and drop them *before* coalescing, or two runs that come out as the same code span will not merge and their backtick delimiters will meet |
| 12 | Code block | Paragraph style named `HTML Preformatted`, `Preformatted Text` (LO), `Source Code` (Pandoc), `Code`, `Plain Text`? no — plus heuristic: paragraph whose every run is monospace; merge consecutive such paragraphs into one fence | ```` ``` ```` fenced block | Inner text raw, no escaping; tabs kept; choose fence longer than any internal backtick run. Language never recoverable — bare fence. **The heuristic carries two guards, and both are load-bearing.** (a) It is switched off entirely where the document's own font *baseline* is monospace — the `w:default="1"` paragraph style folded down its chain, and `w:docDefaults` behind it. A filing set in Courier makes every run monospace and would otherwise convert to one fence with every delimiter dead inside it; the font is a signal only where it tells one run from its neighbours. Word normally puts a theme slot in `w:docDefaults` and the real family on `Normal`, so the paragraph-style half is the likelier one. (b) A run that contributes no visible character abstains rather than voting: the tab and the Zs category (§5.3's list), a `w:t` of nothing but a CR or LF (which folds to a space), or a soft hyphen (dropped). U+00A0 is the one exclusion, because row 35 makes it content. Word splits a logical run at every rsid boundary and the space between two code runs routinely lands in the body font |
...
| **`$`** | Escape as `\$`, but only when the assembled line holds **two or more** `$`; a line with one is left alone | GitHub has read `$...$` as inline math and `$$...$$` as display math since 2022, so `costs $5 and $10` renders `5 and ` in math font and loses both signs — decision **D12 in CLAUDE.md**, ruled 2026-08-26. A span needs two delimiters, so counting them is the whole rule: renderers disagree about whether a space may follow the opener or a digit the closer, and a count is safe under every reading without reproducing any of them. Display math and GitHub's backtick spelling `` $`x`$ `` both need two dollars too, so neither needs a clause. Never in a code span or fence (backslash is literal there and a `$` in code is inert), and never in a link destination (not inline content, and `%24` is not always equivalent to a `$` a server expects) |
...
10. **Dollar-sign math is an extension, not CommonMark**, and GitHub, GitLab, Obsidian, Pandoc and the KaTeX-based previews each spell it slightly differently. Do not reproduce one renderer's opener/closer conditions — count instead, and leave at most one unescaped `$` per line. GitHub's own documentation prescribes `\$` for a literal dollar beside math, so the escape is honoured by the extension itself, and in a renderer with no math at all `\$` is an ordinary punctuation escape that renders as `$`. Two consequences worth stating: the rule is sound only while `\` is escaped *unconditionally* (a source `\` before a `$` would otherwise swallow the inserted escape), and the count must be taken over the whole assembled line — counting per run or per coalesced span would see `costs $5` and `and $10` separately and escape neither. A `$` before a hard break and a `$` after it are two lines and stay unescaped; that is an assumption about inline math not crossing a line end, pinned by `tests/fixtures/dollars`.
11. **Flanking rules break delimiters that touch punctuation**, and the failure is silent. CommonMark will not let a delimiter run open where it is both preceded by a letter and followed by punctuation, nor close where it is both followed by a letter and preceded by punctuation. So `word**(a)**after` emits four literal asterisks and no emphasis at all, and `word~~**x**~~` emits four literal tildes — a `~~` in front of a `**` is *always* followed by punctuation, so a strikethrough that wraps anything fails wherever a letter stands in front of it. Two delimiter runs that meet are one run to a parser, which is why `***T*****=eq=**` fails too: the run of five is preceded by a letter. And `~~a~~~~b~~` is a run of four tildes, which GFM's strikethrough does not recognise at all. §5.3's hoisting is the whitespace half of this same rule; this is the punctuation half, and hoisting cannot fix it because the punctuation is content. **Rule: where the outermost delimiter cannot parse where it stands, emit the HTML element instead** — `<strong>`, `<em>`, `<del>` — which has no flanking rule of any kind. A code span needs no such fallback: its delimiters are backtick runs and they parse regardless of what is beside them. **The flanking classes are not the only trigger, and the other one is arithmetic no character class can express.** Having merged adjacent runs of one delimiter character into a single run, CommonMark then pairs openers to closers by *length* — its rule of three — so three emphasis spans meeting with no text between them can leave a run that no pairing resolves: `**bo*****th****ree*` emits six literal asterisks and loses all three spans, with no punctuation anywhere in it. A span abutted by an identical delimiter run on **both** sides therefore takes the element form as well, which has neither a length nor a flanking rule and also keeps the two Markdown runs apart. Punctuation here is CommonMark's own definition — the Unicode P and S general categories — and a hand-picked subset of it is a trap in both directions: a letter called punctuation costs one HTML element where a delimiter would have done, and a punctuation character called a letter emits a delimiter that does not render. Do not guess the ranges; generate them from the character database. The characters that matter in practice are wider than they look: the Arabic full stop, the Hebrew maqaf, the Greek question mark and the Devanagari danda all end sentences, and all of them sit outside the blocks a Latin-centric table would name.
...
Word happily stores `<w:r><w:rPr><w:b/></w:rPr><w:t xml:space="preserve">bold </w:t></w:r>` — trailing space *inside* the bold run. `**bold **text` does not parse as emphasis (CommonMark left/right-flanking rules: closing `**` cannot be preceded by whitespace; opening `**` cannot be followed by whitespace). **Rule: after coalescing, hoist leading/trailing whitespace out of every formatted span before wrapping**: `**bold** text`. A span that becomes empty or whitespace-only after hoisting emits its whitespace with no delimiters. Same rule for `*`, `~~`, `` ` `` (for code spans keep one interior pad space only for the backtick-collision case), and link text (leading/trailing spaces in `[ text ]` are legal but ugly — trim into surroundings). Hoist U+00A0 (NBSP) too — and in fact the whole Zs category, which is what CommonMark ≥0.30 counts as Unicode whitespace for emphasis flanking: U+0020, U+00A0, U+1680, U+2000–U+200A, U+202F, U+205F and U+3000. A closing `**` preceded by any of them may not parse, and none is exotic — U+2002 is one Insert ▸ Symbol away in Word and U+3000 is what a CJK keyboard's space bar produces. Moving them outside the delimiters is safe in every renderer. U+200B is **not** in the set: it is Cf rather than Zs, and CommonMark does not count it.
...
| **Google Docs export** | Minimal styles.xml (often direct formatting for everything, including headings that *do* use `HeadingN` styles but bold/size directly); no rsids; single-run paragraphs; numbering.xml with one abstractNum per list and bullet glyphs `●`/`○`/`■` in lvlText; images sometimes in `word/media` with generic names; hyperlinks always `w:hyperlink`; no docDefaults surprises; may omit settings.xml parts. *Not observed first-hand*: no Google Docs export was available when M11 built its producer corpus, so `tests/fixtures/gdocslike` is hand-authored to this row and is not an export |
| **LibreOffice** | Styles named `Heading 1` (space) with styleId `Heading1`; `Preformatted Text`, `Source_20_Text` (`_20_` = escaped space in styleId), `Internet Link` character style for hyperlinks and a `Text Body` default in older builds, where 24.2.7 writes `Hyperlink` and `Body Text`; VML sometimes for images in older versions; `ListParagraph` absent — own `ListNumber`/`ListBullet` styles carrying numPr. **Observed in LibreOffice 24.2.7's own exports** (`tests/fixtures/libreoffice`, `libreofficelists`, `libreofficehtml`): the quotation paragraph style is named `Block Quotation` (older builds wrote `Quotations`); `Body Text`, `First paragraph`, `Table Contents` and a bold `Table Heading`; code runs in a `SourceText` character style and code blocks in `Preformatted Text`; `w:jc="left"` on every paragraph, table cells included; `Heading 2` set bold *and* italic in the style; a list item's second paragraph carries `w:numId="0"` and a `w:ind` that lines it up with the item's text, rather than a numbering level of its own; and an HTML `<hr>` imported as a `Horizontal Line` paragraph style whose `w:pBdr` bottom border is in the **style**, with nothing in the paragraph |
| **Pandoc-generated** | Styles `Body Text`, `Compact`, `Source Code`, `Verbatim Char`, `First Paragraph`; clean single runs; footnotes standard. **Observed in pandoc 3.9's exports** (`tests/fixtures/pandoc`, `pandocrules`): `Block Text` for a blockquote and `Image Caption` under a `Captioned Figure`; a list item's second paragraph is a `w:numPr` naming a numbering level whose `w:lvlText` is a single space, and pandoc writes those levels as one definition, a single space at every depth, which the continuations of every list share; a thematic break is `<w:pict><v:rect o:hr="t"/></w:pict>` in an otherwise empty paragraph; media parts are named after their relationship ids (`word/media/rId11.png`) |
...
- **Path traversal**: ZIP entry names and relationship Targets containing `../`, leading `/`, `\`, drive letters (`C:`), NTFS ADS (`name:stream`). Resolve part names purely inside an in-memory package map; when extracting media, generate output filenames yourself (`image1.png`, `image2.jpg` ...) — **never** reuse archive entry paths on disk. Relationship targets are handled from M4 (`OpcResolveTarget` refuses a climb above the root, a scheme and a backslash, and re-checks after percent-decoding). An *entry name* of that shape refuses the whole archive, which is decision **D10 in CLAUDE.md** as M11 answered it: a name that begins with a drive letter, begins with `/`, holds a `\`, holds any other `:`, or has a `.` or `..` segment is refused whether or not anything references the entry, and the message names the rule and the entry. A directory entry's trailing `/` and an empty interior segment (`word//x.xml`) are accepted. LibreOffice 24.2.7 refuses and accepts exactly the same twenty variants M11 tried, and none of the producers M11 could run writes a refused shape. No archive name reaches disk either way.
...
- Encoding: parts are UTF-8 (occasionally UTF-16 with BOM — honor the XML declaration/BOM). Validate UTF-8 and **refuse a part that is not valid UTF-8**, naming the part and which rule the bytes broke (overlong form, encoded surrogate, truncated sequence, out of range) — decision **D8 in CLAUDE.md**, ruled 2026-08-24. `UtfValidate` also computes the offset of the offending sequence, but nothing reports it to the user yet. This paragraph said "replace invalid sequences with U+FFFD rather than aborting" until that ruling, which is the conflict D8 was raised to settle: a refusal is checkable today as an exit code and a message, and refusing first leaves relaxing to U+FFFD open later, while the reverse would break output users already had. U+FFFD substitution survives in exactly one place, the console path in `Utf`, where a path that cannot be represented should still be reportable.
...
```
 ...
 [2] XML tokenize        per needed part -> streaming pull tokens     (namespace-URI aware, no DOM)
